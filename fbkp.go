...
import (
	"fmt"
	mmap "github.com/kenshaw/cznic-ql/common/file"
	"os"
	"path/filepath"
	"strings"
)
...
// to "test.txt.bak"
func CopyFileContents(src, dst string) (int, error) {
	file_in, err := os.OpenFile(src, os.O_RDWR, 0644)
	if err != nil {
		return -1, err
	}
	defer file_in.Close()

	mmap_in, err := mmap.Open(file_in)
	if err != nil {
		return -1, err
	}
	defer mmap_in.Close()

	info, err := mmap_in.Stat()
	if err != nil {
		return -1, err
	}
	perm := info.Mode().Perm()
	...
	file_out, err := os.OpenFile(dst, os.O_RDWR|os.O_CREATE|os.O_TRUNC, perm)
	if err != nil {
		return -1, err
	}
	defer file_out.Close()

	mmap_out, err := mmap.Open(file_out)
	if err != nil {
		return -1, err
	}
	defer mmap_out.Close()

	in_data := make([]byte, size)

	_, err = mmap_in.ReadAt(in_data, 0)
	if err != nil {
		return -1, err
	}

	n, err := mmap_out.WriteAt(in_data, 0)
	if err != nil {
		return -1, err
	}
...
func BackupFile(path, ext string) error {
	...
	if !info.IsDir() {
		...
		_, err := CopyFileContents(path, bkp_path)
		if err != nil {
			return err
		}
	}
...
func RestoreFile(path string) error {
	...
	if !info.IsDir() {
		...
		_, err := CopyFileContents(path, og_path)
		if err != nil {
			return err
		}
	}
